...
while ($master_id = mysqli_fetch_array($masters)) {
    ...
    $schedule_master = mysqli_fetch_row(mysqli_query($link, "SELECT not_work, start_of_work, end_of_work from schedule where id_master=" . $master_id[0] . " and day_of_week=" . $_POST['day_of_week'])); 
...
if (isset($free_master)) {
    ...
    //массив времени с интервалом 1 час
    $schedule = [];
    ...
    $records = mysqli_query($link, "SELECT * from records where master_id=" . $free_master . " and date_record='" . $_POST['date'] . "' and canceled=0");
